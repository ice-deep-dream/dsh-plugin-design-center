...
async function readBoard(ctx, sessionId) {
	...
	if (cwd === void 0) return fail("no-cwd", "session has no working directory");
	try {
...
async function runRender(ctx, args) {
	...
	if (script === null) return fail("not-found", "dev-plan-assistant render_animated_diagram.py not found on this host");
	...
	if (lastExit !== 0) return {
		...
		error: {
			code: "render-failed",
			message: "render command exited non-zero",
			details: outcome
		}
	};
	return ok(outcome);
